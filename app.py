...
def login():
    if request.method == "POST":
        ...
        if role == "admin":
            ...
            if not phone:
                return jsonify({"success": False, "message": "Telefoonkaaga gali!"})
...
def admin_stats():
    ...
    if period == "search" and search_date:
        try:
            start = datetime.strptime(search_date, "%Y-%m-%d")
    ...
    try:
        ...
        for i, row in enumerate(records, start=2):
            if str(row.get("Order_ID","")) == str(order_id):
                current_status = str(row.get("Status","")).lower().strip()

                # If already NOT pending — refuse immediately
                if current_status != "pending":
                    return jsonify({"success": False, "message": "Already processed"})

                # Update Orders status FIRST — blocks race conditions
                ws.update_cell(i, status_col, "Processed")
                cache_del_prefix("sheet:")
                bust_ws_cache()

                # Check Payments — prevent duplicate entries for same Order_ID
                pws       = get_cached_ws(PAYMENTS)
                p_records = pws.get_all_records()
                already_paid = any(
                    str(r.get("Order_ID","")) == str(order_id)
                    for r in p_records
                )
...
def get_menu(category):
    ...
    # Fix local static paths — prepend server host for mobile access
    host = request.host_url.rstrip('/')
    for item in items:
        url = str(item.get("Image_URL",""))
        if url.startswith('/static/'):
            item["Image_URL"] = host + url
        # base64 data URLs work as-is on all devices
...
def add_menu_item():
    try:
        ...
        # Make image URL absolute if it's a local path
        if image_url and image_url.startswith('/static/'):
            # Store relative path — frontend will prepend server URL
            pass
...
def upload_image_local():
    """
    Convert image to base64 data URL — works on ALL devices including mobile.
    No server path needed — image travels with the data.
    Google Sheets stores the base64 string directly.
    ...
    try:
        ...
        # Resize to max 400x400 for Sheets compatibility
        max_size = 400
        ...
        # Check size — Google Sheets cell max ~45KB base64
        if len(compressed) > 33000:  # ~45KB base64 limit
            # Extra compression
            output2 = io.BytesIO()
            img.thumbnail((250, 250), PILImage.LANCZOS)
            img.save(output2, format="JPEG", quality=55, optimize=True)
        ...
        try:
            ...
            return jsonify({"success": False, "message": str(e2)})
...
def customer_place_order():
    data       = request.json
    # Use session phone if customer didn't provide one
    phone = data.get("phone","").strip() or session.get("customer_phone","")
    # Also use session name if not provided
    full_name = data.get("full_name","").strip() or session.get("customer_name","")
    items_json = json.dumps(data.get("items",[]))
    total      = safe_float(data.get("total",0))
    ...
    cache_del_prefix("sheet:")
    bust_ws_cache()
    ws.append_row([order_id, full_name, phone,
...
@customer_required
def customer_orders():
    all_orders = cached_sheet(ORDERS, ttl=CACHE_TTL["orders"])
    # Filter by customer phone — privacy
    phone = session.get("customer_phone","")
    if phone:
        my_orders = [o for o in all_orders
                     if str(o.get("Phone","")).strip() == phone]
    else:
        my_orders = []
